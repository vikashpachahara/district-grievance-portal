...
def complaint(department, category):
    ...
    if request.method == "POST":
        ...
        # Geocoding: Turn GPS into Readable Address
        ...
        cur = conn.cursor()
        cur.execute("INSERT INTO complaints (tracking_id, user_id, department, category, incident, description, address, file, priority, status, date, time, latitude, longitude) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                   (tracking, session["user"], department, category, category, description, readable_address, filename, set_priority(category), "Active", str(datetime.date.today()), datetime.datetime.now().strftime("%H:%M:%S"), lat, lon))
        conn.commit()
        cur.close()
        conn.close()
        return redirect(url_for('my_complaints'))
