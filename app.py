...
# Pulls the connection string from Render's Environment Variables
DATABASE_URL = os.environ.get("DATABASE_URL")

# We no longer need to save to disk for Render free tier!
UPLOAD_FOLDER = "static/uploads"
os.makedirs(UPLOAD_FOLDER, exist_ok=True)

def get_db_connection():
    # 5-second timeout added for stability
    conn = psycopg2.connect(DATABASE_URL, sslmode='require', connect_timeout=5)
    return conn

# ---------------- REVERSE GEOCODING (Map Brain) ----------------
# Turns GPS coordinates into a human-readable address.
...
# ---------------- DATABASE INIT ----------------
# The structure of your database "brain"
def init_db():
...
def complaint(department, category):
    ...
    if request.method == "POST":
        ...
        # 1. Capture the exact GPS coordinates from the form
        lat = request.form.get("latitude")
        lon = request.form.get("longitude")

        # 2. Render Ephemeral Storage Fix: Convert Image to permanent Base64 string
        # to save DIRECTLY inside your Supabase database.
        file_data = ""
        if file and file.filename != "":
            encoded_string = base64.b64encode(file.read()).decode('utf-8')
            # Construct a complete data URI for easy HTML embedding
            file_data = f"data:{file.content_type};base64,{encoded_string}"
        
        # 3. Geocode: Turn raw GPS into a written, readable address
        ...
        # 4. Save everything, including coordinates, to Supabase
...
def admin_dashboard():
    ...
    cur.close()
    conn.close()
    # DictCursor allows 'row['column']' syntax for clarity
